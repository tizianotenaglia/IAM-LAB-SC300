...
Security defaults and Conditional Access are mutually exclusive by design
— CA is meant to fully replace security defaults with granular, customizable policies,
 which security defaults can't provide since they're just an on/off switch.
...
the two break glass accounts should already be excluded from any future CA policy, so 
